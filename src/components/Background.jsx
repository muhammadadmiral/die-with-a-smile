...
const Background = ({ isPlaying, audioLevel = [], currentLyric, isChorus, onLoad }) => {
  ...
  useEffect(() => {
    ...
    let bassSum = 0, subBassSum = 0, midSum = 0;
...
const SpaceScene = ({ 
  ...
  useFrame((_, delta) => {
    ...
    if (nebulaRef.current && isPlaying) {
      ...
      const breathScale = deviceConfig.NEBULA_SCALE * 
...
// Add this function to create chorus transition effects
const ChorusTransitionEffect = ({ isActive, position = [0, 0, -10] }) => {
  const meshRef = useRef();
  
  useEffect(() => {
    if (meshRef.current && isActive) {
      // Create explosive animation when chorus starts
...
const ChorusEffects = ({ isChorus, audioIntensity, isPlaying, currentLyric }) => {
  ...
  useEffect(() => {
    ringsRef.current = Array(5).fill().map((_, i) => ({
      radius: (i + 1) * 2,
  ...
  return (
    <group visible={isChorus || chorusTransition}>
      ...
      <ChorusTransitionEffect isActive={chorusTransition} />
      ...
      {/* Expanding rings */}
      {isChorus && ringsRef.current.map((ring, i) => (
        <ExpandingRing 
          key={`chorus-ring-${i}`}
          radius={ring.radius}
          width={ring.width}
        ...
        >
          <Sphere args={[1.5, 32, 32]} position={[0, 0, 0]}>
            ...
            >
              <GradientTexture
                stops={[0, 1]}
                colors={[CONFIG.COLORS.CHORUS.PRIMARY, CONFIG.COLORS.CHORUS.SECONDARY]}
              />
            </MeshDistortMaterial>
          </Sphere>
      ...
      {isChorus && currentLyric && currentLyric.toLowerCase().includes("smile") && (
...
const LyricAccentBurst = ({ intensity }) => {
  ...
  useFrame(() => {
    if (!burstRef.current || !particlesRef.current) return;
    
    const elapsed = (Date.now() - startTimeRef.current) / 1000;
    
    // Expand and fade out
    burstRef.current.scale.set(
      1 + elapsed * 2,
      1 + elapsed * 2,
      1 + elapsed * 2
    );
...
const SolarSystem = ({ 
  ...
  const solarSystemData = useMemo(() => {
    ...
    for (let i = 0; i < planetCount; i++) {
      ...
      const planetColor = isVerseColor 
        ? CONFIG.COLORS.VERSE.PRIMARY 
        : CONFIG.COLORS.CHORUS.PRIMARY;
      const planetHighlightColor = isVerseColor 
  ...
  return (
    <group position={[0, 0, 0]}>
      ...
      <mesh 
        ref={sunRef}
        position={solarSystemData.sun.position}
      >
      ...
      {!isLowPerformance && !isMobile && solarSystemData.planets.map((planet, i) => (
        <mesh 
          key={`orbit-${i}`}
