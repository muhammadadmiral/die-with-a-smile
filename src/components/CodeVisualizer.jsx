...
const CodeVisualizer = ({ currentTime, isPlaying, currentLyric }) => {
...
await forever.promise(); // "like it's the last"`
  ...
  useEffect(() => {
    ...
    const lyricsInChorus = chorusLines.some(line => 
      currentLyric.includes(line) || 
      line.includes(currentLyric)
    );
  ...
  const triggerCodePulse = () => {
    ...
    setCodePulse(true);
    setTimeout(() => setCodePulse(false), 1000);
  };
  ...
  // Apply syntax highlighting when snippet changes
  useEffect(() => {
    if (codeRef.current) {
      try {
  ...
  const renderCodeWithTypewriter = () => {
    ...
    return codeLines.map((line, index) => {
      ...
      if (index === activeTypingLine) {
        ...
        return (
          <div key={`${currentSnippet}-${index}`} style={getLineStyle(index, codeLines)} className="line">
            <span dangerouslySetInnerHTML={{ __html: Prism.highlight(visiblePart, Prism.languages.javascript, 'javascript') }} />
      ...
      return (
        <div key={`${currentSnippet}-${index}`} style={getLineStyle(index, codeLines)} className="line">
          <span dangerouslySetInnerHTML={{ __html: Prism.highlight(line, Prism.languages.javascript, 'javascript') }} />
        </div>
      );
    });
  };
  ...
  useEffect(() => {
    if (!isInitializedRef.current) {
      console.log("CodeVisualizer initialized");
      isInitializedRef.current = true;
    }
  }, [isChorus, currentSnippet, isTypewriting]);
  ...
  return (
    <motion.div
      ...
      className="fixed bottom-4 right-4 max-w-md z-20 pointer-events-none p-4"
        ...
        {codeVisible && (
          <motion.div
            ...
            transition={{
              ...
              scale: {
                repeat: (isPlaying && !codePulse) ? Infinity : 0,
                duration: 0.8,
              },
              x: {
                duration: 0.2,
          ...
          >
            ...
            <div className="absolute bottom-2 left-2 px-2 py-0.5 bg-dark-800/50 rounded-full text-[10px] flex items-center">
              <span className="text-gray-300">{
                currentSnippet === "default" ? "Main" :
                currentSnippet === "chorus" ? "Chorus" :
                currentSnippet === "bridge" ? "Bridge" : "Outro"
              }</span>
            </div>
          </motion.div>
        )}
